  ...
  return (
    <PageTransition>
      <div className="flex flex-col min-h-full">
        <Header title="Institutional Registry" />
          ...
          <div className="flex items-center justify-between">
            <div className="relative">
              ...
              <Input
                ...
                className="pl-9 w-[300px] border-slate-200 shadow-sm rounded-xl bg-white focus-visible:ring-blue-600"
                value={search}
            ...
            <div className="flex items-center gap-3">
               <Dialog open={isImportOpen} onOpenChange={setIsImportOpen}>
                <DialogTrigger render={
                  <Button variant="outline" className="h-11 rounded-2xl border-slate-200 font-bold text-xs gap-2 px-6 shadow-sm bg-white hover:bg-slate-50">
                    <UploadCloud className="w-4 h-4 text-blue-600" /> Bulk Import
                  </Button>
                } />
                <DialogContent className="sm:max-w-[450px] rounded-[2.5rem] p-0 overflow-hidden bg-white border border-slate-200 shadow-2xl">
                  <div className="p-8 space-y-6">
                    <DialogHeader>
                      <DialogTitle className="text-2xl font-black italic">INGEST DATA</DialogTitle>
                    ...
                    <div className="bg-slate-50 border border-slate-100 p-6 rounded-3xl space-y-3 mb-2">
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Required CSV Columns</p>
                        <div className="flex flex-wrap gap-2">
                            {['name', 'roll', 'email', 'parent_email'].map(col => (
                        ...
                        <p className="text-[9px] text-slate-400 font-bold leading-relaxed mt-2 pt-2 border-t border-slate-100">
                           <span className="text-blue-600">Note:</span> Roll numbers must follow BU format (e.g., U03FS23S0001).
                        </p>
                    </div>

                    <div className="space-y-4">
                      <Label htmlFor="csv-upload" className="block p-10 border-2 border-dashed border-slate-200 rounded-3xl hover:border-blue-400 hover:bg-blue-50/30 transition-all cursor-pointer group text-center bg-slate-50/30">
                        <div className="flex flex-col items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-[10px] text-slate-400 font-medium mt-1">Accepts .CSV files up to 10MB</p>
                          </div>
                        </div>
                    ...
                    <Button 
                        variant="ghost" 
                        className="w-full h-12 rounded-2xl text-[10px] font-black uppercase tracking-widest text-blue-600 border border-blue-50 hover:bg-blue-50 gap-2"
                        onClick={() => {
                            const content = "name,roll,email,parent_email\nJohn Doe,U03FS23S0001,john@gmail.com,parent@gmail.com";
                            const blob = new Blob([content], { type: 'text/csv' });
                            const url = window.URL.createObjectURL(blob);
                            const a = document.createElement('a');
                            a.href = url;
                            a.download = 'attendly_student_template.csv';
                            a.click();
                        }}
                    >
              ...
              <Dialog open={isAddOpen} onOpenChange={setIsAddOpen}>
                <DialogTrigger render={
                    <Button 
                        ...
                        className="h-11 rounded-2xl bg-blue-600 text-white font-black uppercase tracking-widest hover:bg-blue-700 shadow-xl shadow-blue-500/10 px-6 gap-2"
                    >
                        <Plus className="w-4 h-4" /> Enroll Student
                    </Button>
                } />
                <DialogContent className="sm:max-w-[500px] rounded-[2.5rem] p-0 overflow-hidden bg-white border border-slate-200">
                  <div className="p-8">
                    <DialogHeader className="mb-8 text-center">
                      <DialogTitle className="text-2xl font-black italic uppercase">New Enrollment</DialogTitle>
                      <DialogDescription className="font-bold text-slate-400">Enter personal and academic coordinates.</DialogDescription>
                    </DialogHeader>
                    <div className="grid grid-cols-2 gap-6">
                      <div className="space-y-2 col-span-2">
                        <Label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Full Identity</Label>
                        <Input placeholder="John Doe" className="h-12 rounded-2xl border-slate-200 font-bold" value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} />
                      </div>
                      <div className="space-y-2">
                        <Label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Roll Number</Label>
                        <Input placeholder="U03FS23S0134" className="h-12 rounded-2xl border-slate-200 font-bold" value={formData.roll} onChange={e => setFormData({ ...formData, roll: e.target.value })} />
                      </div>
                      <div className="space-y-2">
                      ...
                      <div className="space-y-2 col-span-2">
                        ...
                        <Input placeholder="student@edu.com" className="h-12 rounded-2xl border-slate-200 font-bold" value={formData.email} onChange={e => setFormData({ ...formData, email: e.target.value })} />
                      </div>
                    </div>
                  </div>
                  <div className="p-8 pt-0 flex flex-col gap-3">
                    <Button className="h-14 rounded-2xl bg-slate-900 text-white font-black uppercase tracking-widest shadow-2xl shadow-slate-900/10" onClick={handleCreate}>Finalize Enrollment</Button>
                    <Button variant="ghost" className="h-12 rounded-xl text-slate-400 font-bold text-xs" onClick={() => setIsAddOpen(false)}>Discard</Button>
                  </div>
                </DialogContent>
              </Dialog>
            </div>
